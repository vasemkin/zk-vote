...
describe("ZKTree Smart contract test", () => {
    ...
    it("Testing the verifier circuit", async () => {
        const commitment = await generateCommitment(BigNumber.from(1));
        ...
        assert.equal(publicSignals[2], rootAndPath.root);

        const vKey = JSON.parse(
            fs.readFileSync("build/Verifier_vkey.json").toString()
        );
        ...
        const verifyRes = await verifier.verifyProof(
            cd.a,
            cd.b,
            cd.c,
            cd.input
    ...
    it("Should calculate commitment and nullifier hash correctly", async () => {
        const res = await generateCommitment(BigNumber.from(1));
        // console.log(res)

        const { proof, publicSignals } = await snarkjs.groth16.fullProve(
            { nullifier: res.nullifier, secret: res.secret, score: res.score },
        ...
        assert.equal(publicSignals[2], res.score);
    });
    ...
    it("Should calculate hash correctly", async () => {
        ...
        assert.equal(
            res,
            ethers.BigNumber.from(mimc.F.toString(res2)).toHexString()
        );
    });
    ...
    it("Should calculate the root correctly", async () => {
        ...
        assert.equal(
            ethers.BigNumber.from(res).toHexString(),
            ethers.BigNumber.from(res2.root).toHexString()
        );
    });
    ...
    it("Should calculate the root and proof correctly after commit 3.", async () => {
        ...
        const res2 = calculateMerkleRootAndPath(
            mimc,
            TREE_LEVELS,
            [1, 2, 3],
            3
        );
        const root = checkMerkleProof(
            mimc,
            TREE_LEVELS,
            res2.pathElements,
            res2.pathIndices,
        ...
        assert.equal(
            ethers.BigNumber.from(res).toHexString(),
            ethers.BigNumber.from(root).toHexString()
        );
    });
