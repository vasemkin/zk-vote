...
export async function generateCommitment(
	...
	const commitment = mimc.F.toString(
		mimc.multiHash([nullifier, secret, score])
	);
...
export function convertCallData(calldata) {
	...
	const c = [argv[6], argv[7]] as [
		PromiseOrValue<BigNumberish>,
		PromiseOrValue<BigNumberish>
	];
	const input = [argv[8], argv[9], argv[10]] as [
		PromiseOrValue<BigNumberish>,
		PromiseOrValue<BigNumberish>,
		PromiseOrValue<BigNumberish>
	];
...
export async function calculateMerkleRootAndZKProof(
	...
	return {
		...
		score: publicSignals[1],
		root: publicSignals[2],
		proof_a: cd.a,
		proof_b: cd.b,
		proof_c: cd.c,
	};
...
export async function calculateMerkleRootAndZKProofLocal(
	levels: number,
	commitments: string[],
	commitment: any,
	wasmPath: string,
	...
	const rootAndPath = calculateMerkleRootAndPath(
		mimc,
		levels,
		commitments.map((strC) => BigNumber.from(strC)),
		commitment.commitment
	);
	const { proof, publicSignals } = await snarkjs.groth16.fullProve(
		{
	...
	console.log({ proof, publicSignals, rootAndPath });
	const cd = convertCallData(
		await snarkjs.groth16.exportSolidityCallData(proof, publicSignals)
	);
	return {
